...
| Help and requests | Signed-out Help and Legal routes exist. Reports can include reviewed exact-card context and optional app/OS/update diagnostics. Drafts are device-local and account-scoped; opening the designated email address does not mark a report sent. | The designated channel does not establish private ticket delivery, server receipt, monitoring, a response procedure or a fulfilled access/deletion request. |
...
5. Monitoring, response ownership, identity verification, secure handling and receipt procedure for requests sent to `berridge14@icloud.com`.
