...
7. Verify the approved delivery scope before activation: current price/history reads use public gateway routes. A private-only provider grant is not sufficient for anonymous delivery; such a grant requires tested server-enforced owner access before provider data is written into those readable stores. Enable the worker and canonical publisher in a bounded canary, then enable backend `MARKET_PRICE_REFRESH_ENABLED` only when that worker is operational. Confirm one authenticated exact-variant request drains, individual sale evidence is retained, the canonical write is acknowledged, and the same variant/condition/currency is readable through the permitted gateway route. A 200 response with no evidence is not success. Inspect the source links, item prices, dates, counts, freshness and chart history before expanding beyond the canary.
...
Code and fixture tests do not make the live feed “super accurate.” At inspection, production had no individually evidenced sold rows. The permission attachment, passing real benchmark, target migration rehearsal, deployment and bounded live canary still require evidence before that claim can change. Provider-observed completion is also not a guarantee against later cancellation/refund, which PokeTrace does not expose in the reviewed contract.
