...
## Stage 11 Gateway Variables

Cloudflare public/non-secret configuration names:
...
- `STACKR_OBSERVABILITY_EVENTS_ENABLED`
...
The Expo client sends the signed-in user's short-lived Supabase access token and a random persisted device identifier. It contains no gateway origin key, recognition service secret, backend admin key, provider credential or service-role key.

## Stage 13 CI/CD Variables

GitHub protected environment secrets:
...
GitHub protected environment variables:
...
- `SUPABASE_LOCAL_BASELINE_READY`
- `STACKR_CATALOGUE_INGESTION_SCHEDULED`
- `STACKR_CATALOGUE_INGESTION_AUTOMATION_APPROVED`

Workflow-only values generated by GitHub Actions:

- `STACKR_RELEASE_REQUEST_ID`
- `STACKR_ROLLBACK_TARGET_ID`
- `STACKR_ROLLBACK_REASON`

The database URL and all provider tokens are server/CI-only. GitHub environment reviewers should protect production values. Approval variables are release evidence gates, not substitutes for a successful test result.
