...
Production already contains all **513 approved staging set visuals** by exact asset ID (295 logos, 173 symbols, 45 covers). Matching staging links did not establish API visibility: a further production audit found **139 existing Japanese set logos** without links to the current publication. Their exact current-version links are now restored, using their existing Supabase objects. The app reads these through the asset manifest alongside its bundled set artwork; the `/v1/sets` response itself does not contain logo fields. [Logo recovery receipt](../../deploy/evidence/api-bridge-japanese-set-logos-20260908.json).

The same audit of approved production card images found **19 existing English Scarlet & Violet Promo images** with exact current identities but no current-version links. Those links are now restored without changing the assets or their existing optimised derivatives. [English promo receipt](../../deploy/evidence/api-bridge-english-svp-card-images-20260908.json). The [current image-link audit](../../deploy/evidence/current-card-image-link-audit-20260908.json) records the scope and held off-publication rows. Production has more populated English metadata than staging; replacing it would lose existing work.
...
Focused search and API integration checks, backend type checking, lint and the protected release workflow validate the code. Final production probes and workflow IDs belong in the release receipt. For the PokeData image rollback, delete only recorded current-version pairs, then inserted asset IDs, then the new source only if it has no remaining references. For the Japanese logos and English promos, remove only the recorded new links; their pre-existing assets, derivatives and sources must remain. Do not delete by set or provider predicates.
