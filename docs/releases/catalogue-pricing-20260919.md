...
Release preparation now closes the previous implementation gaps: stored valuation preparation is independent of the catalogue worker; the API defaults off until its schema/worker is ready; private comparable trend points are published atomically (30-minute buckets, 90-day retention, bounded 7/30-day reads), with repeated provider evidence, partial and stale generations excluded from new points. Quantity, source, basis and identity changes create separate trend scopes. Alias insert/update/delete signals include same-version repairs and set-level aliases. Master-set units deduplicate canonical finish slots and refuse ambiguous duplicate identities. Read-only inspection of actual binder columns found edition, language, card mode and master-set selection, with no saved per-finish inclusion/exclusion configuration; both standard and master summaries remain available for the client's chosen mode.

Remaining acceptance evidence: representative target-schema validation; live throughput/lease duration for very large collections; authenticated API performance (ownership-revision checks still read complete private input); real Home/Binder and account-switch behaviour on the installed device. The full sweep additionally requires verified numeric provider capacity and a measured schedule that fits 12 hours. A new fail-closed preflight refuses activation when even the minimum runtime or request budget cannot fit. Passing that arithmetic guard alone is not proof of throughput or provider permission.

## Next production release preparation — 19 September

[PR #207](https://github.com/tberridge86/Stackr/pull/207) is the source candidate, queued in [next TestFlight inclusion](../pricing/next-testflight-release.md). Release owns promotion. App version remains **1.0.3**, profile `production-owner`, channel `owner-recognition`, runtime `1.0.3-owner-recognition-v1`. No remote state or build number changed.

Local checks passed after the readiness fixes: app/backend typecheck; lint (0 errors, 12 existing warnings); catalogue SQL/worker/HTTP and stable-value/trend tests; owner worker, personal pricing and market service; Home release, collection UI, Binder retrieval; mobile runtime and owner configuration; gateway 44/44; API contract 40/40; repository secret scan. SQL integration now also executes alias-only repair signals and verifies owner-isolated history, repeated-quote deduplication and default-off/dry-run behaviour.

Production-owner iOS JavaScript export succeeded into `.tmp/catalogue-pricing-release-ios`. Hermes bundle `entry-a094cf89627366fdde2f44d50add6581.hbc`, SHA-256 `d6b3a0061139527ad1b49e7b0f59acd42d61c31ecb83ee6cb564d076cc2c0340`. Export secret scan passed over 592 candidate files. Export is local bundling evidence, not native signing, TestFlight installation or device acceptance. Final-commit CI is recorded on the PR.

### Stored valuation rollout for the next release

This lane can use existing bounded provider workers without starting catalogue-wide fetching. Defaults are safe before migration: `STACKR_PREPARED_VALUATIONS_ENABLED` absent/false returns authenticated 404 without calling new RPCs; existing Home fallback remains available. Binder's prepared subtotal stays pending until enabled, so finish this backend rollout before shipping the new Binder path.

1. Reconcile the database ledger and rehearse/review the named additive migration against the target schema; apply via the established release process. Verify service-only grants and alias trigger write permissions, retain existing quote history.
2. Deploy the reviewed API, gateway and existing worker source. Keep `STACKR_CATALOGUE_PRICING_ENABLED=false`, `STACKR_TCGDEX_CAPACITY_VERIFIED=false`, and `STACKR_PREPARED_REFRESH_QUEUE_ENABLED=false`. Preserve the existing bounded worker commands, schedules and provider approvals. Do not set the catalogue scheduler selector for this lane.
3. Enable `STACKR_PREPARED_VALUATIONS_ENABLED=true` on the existing worker(s) and API. The five-minute queue worker now prepares complete stored summaries after its bounded queue pass; it can publish even when no provider queue item is present. Read the owner endpoint to request a generation, observe an actual publication and verify its exact quantities, known subtotal and reconciled coverage. No stored-summary read calls providers.
4. To activate whole-collection manual/new-holding queue enumeration, release must separately review capacity of the existing queue and set `STACKR_PREPARED_REFRESH_QUEUE_ENABLED=true` on the preparing worker. This authorizes bounded queue insertion (50 identities per preparation), not the full catalogue sweep. If false, the summary reports blocked identities honestly. Preserve the existing queue consumer limits and monitor backlog before enabling.
5. Measure authenticated cold/warm endpoint timings, payload size and errors; test same-account reopen, partial failure, removal, quantity changes, standard/master switching and sign-out on the owner device. Record current API/gateway/client rollback identities before promotion. Then include the client through the coordinated next production-owner release.
