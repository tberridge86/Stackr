...
**Trial Production Baseline Migrations** accepts only the protected restore-branch URL, checks that its project ref differs from both staging and production, restores the checksum-pinned baseline, and replays the repository migrations. Runs `30548860049` and `30551864110` both completed the former 76-migration chain; that historical candidate has 76 history entries and 179 tables. The current repository contains 78 migrations, so a fresh isolated rehearsal is required before promotion. Never point this workflow at staging or production.
