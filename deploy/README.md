...
`deploy/evidence/staging-migration-reconciliation-2026-07-30.json` records the hash and schema-inventory comparison. Three repository migrations are accounted for on current staging, 17 entries are staging-only support/rehearsal changes, and 76 repository migrations remain unverified there. The missing production-era schema baseline was captured read-only, checksum-pinned, and replayed through the then-current chain on an isolated branch. GitHub Actions run `30554631150` produced a candidate with exactly 78 repository migration entries and 179 tables. The repository now contains 79 migrations, so that candidate is one migration stale. Current staging was not mutated and production was not mutated.

The security-hardened candidate is not promotable yet. It is one migration behind the current chain. Supabase advisors reported zero errors, 9 accepted warnings and 15 informational notices on that production-like candidate, down from 7 errors, 33 warnings and 17 informational notices. The remaining warnings are intentional guarded SECURITY DEFINER functions required by current authenticated workflows or RLS. Replacing staging would still discard an authorised 120-printing catalogue/provenance dataset unless it is migrated deliberately. `scripts/deploy/verify-staging-migration-reconciliation.mjs --require-aligned` and `scripts/deploy/verify-staging-readiness-evidence.mjs --require-release-ready` therefore continue to fail closed.
