create index if not exists card_embeddings_dinov2_vits14_384_benchmark_scope_idx
  on ml.card_embeddings_dinov2_vits14_384 (model_id, index_version_id, language_code)
...
begin
  ...
  -- The service-role benchmark needs deterministic embedding-quality ranks,
  -- not approximate HNSW recall. Materialising the exact index/language slice
  -- prevents PostgreSQL from traversing the unfiltered global HNSW graph.
  return query execute pg_catalog.format(
    $search$
      with eligible as materialized (
        select e.variant_id, e.embedding
        from ml.card_embeddings_dinov2_vits14_384 e
        where e.model_id = %2$L::text
          and e.index_version_id = %3$L::uuid
          and e.deprecated_at is null
          and (%4$L::text is null or e.language_code = %4$L::text)
      ),
      nearest as materialized (
        select
          eligible.variant_id,
          (eligible.embedding OPERATOR(extensions.<=>) %1$L::extensions.vector)::double precision as cosine_distance
        from eligible
        order by eligible.embedding OPERATOR(extensions.<=>) %1$L::extensions.vector
        limit %5$s
      )
      select
        c.canonical_key,
