-- Expose explicit image availability and same-artwork fallbacks in the published API.
...
comment on column api.catalogue_cards.native_image_status is
  'Whether the variant has a native image, an explicit same-artwork fallback, or remains missing.';

comment on column api.catalogue_cards.same_artwork_as_variant_id is
  'Variant whose approved asset may be reused when native_image_status is same_artwork_reference.';
