...
from .repositories import CandidateRepository, ModelRegistryEntry
from .schemas import (
    ComponentScores,
    EmbedRequest,
    EmbedResponse,
...
class RecognitionPipeline:
    ...
    async def identify(self, request: IdentifyRequest, request_id: str | None) -> IdentifyResponse:
        scan_id = uuid4()
        ...
        source_type = "device_embedding" if request.embedding else "private_image_key"
        ...
        if failures:
            ...
            await self._record_identify_diagnostics(
                ...
                private_image_key=request.privateImageKey,
                diagnostic_payload={"qualityFailures": failures},
            )
            return response
        ...
        response = self._response_from_scored(
            ...
            reasons=sorted(set([*reasons, *global_flags])),
        ...
        await self._record_identify_diagnostics(
            ...
            source_type=source_type,
            private_image_key=request.privateImageKey,
            diagnostic_payload={"fallbackImageSha256": image_hash},
        )
        return response
    ...
    async def embed(self, request: EmbedRequest) -> EmbedResponse:
        ...
        image, image_hash = await self._load_and_normalise_image(request.privateImageKey, corners=request.corners)
        try:
            embedding = self.model.embed(image.image)
        except ModelUnavailable as exc:
            raise HTTPException(
    ...
    async def _load_and_normalise_image(self, key: str | None, *, corners):
        if not key:
            raise HTTPException(
                status_code=422,
        ...
        try:
            data = await self.storage.read_private_object(key)
