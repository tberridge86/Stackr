...
if (migrationReconciliation.status === 0) {
  ...
  // The repository ledger is eleven reviewed migrations ahead of the last
  ...
  // conflict-deduplication lookup index, launch conflict-report repair, the
  // staging atomic-bootstrap draft capture, its reviewed provenance-only
  // superseder, and the concept-conflict correction.
  ...
  assert.equal(reconciliation.localMigrationFileCount, reconciliation.stagingMigrationHistoryCount + 11);
  ...
  assert.equal(
    ...
    '20260831210059_fix_atomic_official_japanese_metadata_bootstrap_concept_conflict.sql',
  );
