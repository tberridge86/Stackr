...
const backendRailwayConfig = JSON.parse(
  readFileSync("backend/railway.json", "utf8"),
);
...
const recognitionRailwayConfig = JSON.parse(
  readFileSync("recognition-service/railway.json", "utf8"),
);
for (const watchPattern of [
  "recognition-service/**",
  "app/**",
...
const releaseManifest = JSON.parse(
  readFileSync("deploy/release-manifest.json", "utf8"),
);
...
const catalogueWorkerSecretsTemp = mkdtempSync(
  path.join(tmpdir(), "stackr-catalogue-worker-secrets-test-"),
);
try {
  const outputPath = path.join(
    catalogueWorkerSecretsTemp,
    "worker-secrets.json",
  );
  ...
  assert.equal(
    ...
    0,
    workerSecrets.stderr || workerSecrets.stdout,
  );
  assert.deepEqual(
  ...
  for (const value of Object.values(catalogueSecrets)) {
    assert.doesNotMatch(
      workerSecrets.stdout,
      new RegExp(value),
      "worker secret values must not be logged",
    );
  }
} finally {
...
const stagingEvidence = run(
  "scripts/deploy/verify-staging-readiness-evidence.mjs",
);
assert.equal(
  stagingEvidence.status,
  0,
  stagingEvidence.stderr || stagingEvidence.stdout,
);
const evidenceGuardTemp = mkdtempSync(
  path.join(tmpdir(), "stackr-readiness-evidence-test-"),
);
try {
  const sourceEvidence = JSON.parse(
    readFileSync("deploy/evidence/staging-readiness-2026-07-30.json", "utf8"),
  );
  ...
  assert.notEqual(
    spoofedReady.status,
    0,
    "a ready label must not override blocked capture evidence",
  );
  ...
  const tamperedChecksumPath = path.join(
    evidenceGuardTemp,
    "tampered-checksum.json",
  );
  writeFileSync(
  ...
  assert.notEqual(
    tamperedChecksum.status,
    0,
    "tampered benchmark evidence must fail closed",
  );
  assert.match(
    tamperedChecksum.stdout,
    /model_benchmark_evidence_checksum_mismatch/,
  );

  const staleProductionHistoryPath = path.join(
    evidenceGuardTemp,
    "stale-production-history.json",
  );
  const latestEvidence = JSON.parse(
    readFileSync("deploy/evidence/staging-readiness-2026-08-11.json", "utf8"),
  );
  writeFileSync(
    staleProductionHistoryPath,
    JSON.stringify({
      ...latestEvidence,
      supabase: {
        ...latestEvidence.supabase,
        productionMigrationHistoryCount:
          latestEvidence.supabase.localMigrationFileCount - 1,
      },
    }),
  );
  ...
  assert.notEqual(
    staleProductionHistory.status,
    0,
    "stale production migration history must fail closed",
  );
  assert.match(
    staleProductionHistory.stdout,
    /production_migration_history_count_drift/,
  );
} finally {
  rmSync(evidenceGuardTemp, { recursive: true, force: true });
}
const migrationReconciliation = run(
...
if (migrationReconciliation.status === 0) {
  assert.equal(
    migrationAlignmentGate.status,
    0,
    migrationAlignmentGate.stderr || migrationAlignmentGate.stdout,
  );
  assert.doesNotMatch(
    migrationAlignmentGate.stdout,
    /migration_history_not_aligned/,
  );
} else {
  ...
  assert.equal(
    reconciliation.localMigrationFileCount,
    reconciliation.stagingMigrationHistoryCount + 1,
  );
  ...
  const localMigrations = readdirSync("supabase/migrations")
    ...
    .sort();
  assert.equal(
    localMigrations.at(-1),
    "20260813135412_premium_seller_access_boundary.sql",
  );
  assert.notEqual(
    migrationAlignmentGate.status,
    0,
    "global deployment must remain blocked while staging evidence trails",
  );
}
const stagingReleaseGate = run(
  "scripts/deploy/verify-staging-readiness-evidence.mjs",
  ["--require-release-ready"],
);
assert.notEqual(
  stagingReleaseGate.status,
  0,
  "staging evidence must block release until recovery and model gates pass",
);
assert.doesNotMatch(stagingReleaseGate.stdout, /storage_recovery_not_verified/);
assert.doesNotMatch(
  stagingReleaseGate.stdout,
  /database_recovery_not_verified/,
);
...
assert.notEqual(
  releasePreflight.status,
  0,
  "release preflight must fail closed without approvals and credentials",
);
...
assert.match(
  credentialledReleasePreflight.stdout,
  /release_gate_not_ready:activeModelSelected/,
);
assert.match(
  credentialledReleasePreflight.stdout,
  /release_gate_not_ready:activeIndexValidated/,
);
assert.doesNotMatch(
  credentialledReleasePreflight.stdout,
  /release_gate_not_ready:migrationHistoryAligned/,
);
assert.doesNotMatch(
  credentialledReleasePreflight.stdout,
  /release_gate_not_ready:storageBackupVerified/,
);
assert.doesNotMatch(
  credentialledReleasePreflight.stdout,
  /release_approval_missing/,
);
const crossedProjectPreflight = run(
  "scripts/deploy/preflight.mjs",
  ["--release"],
  {
    ...completeStagingEnvironment,
...
assert.match(
  crossedProjectPreflight.stdout,
  /supabase_project_ref_mismatch:staging/,
);
...
for (const variable of [
  ...
  assert.match(
    ...
    new RegExp(`missing_release_variable:${variable}`),
  );
}
assert.doesNotMatch(
  catalogueProductionPreflight.stdout,
...
const modelGate = run("scripts/deploy/verify-model-release.mjs", [
  "--require-active",
]);
assert.notEqual(
  modelGate.status,
  0,
  "model release gate must reject the currently unselected model/index",
);

const secretScan = run("scripts/deploy/secret-scan.mjs");
assert.equal(secretScan.status, 0, secretScan.stderr || secretScan.stdout);

const secretScanTemp = mkdtempSync(
  path.join(tmpdir(), "stackr-secret-scan-test-"),
);
try {
  writeFileSync(
  ...
  const scannedDiagnostic = run("scripts/deploy/secret-scan.mjs", [
    `--directory=${secretScanTemp}`,
  ]);
  assert.notEqual(
    scannedDiagnostic.status,
    0,
    "backup diagnostics containing a Supabase access token must not be uploaded",
  );
...
const rollbackTool = readFileSync(
...
const productionMonitorWorkflow = readFileSync(
...
const productionBaselineWorkflow = readFileSync(
  ".github/workflows/capture-production-schema-baseline.yml",
  "utf8",
);
const baselineMigrationTrialWorkflow = readFileSync(
  ".github/workflows/trial-production-baseline-migrations.yml",
  "utf8",
);
const catalogueTransferWorkflow = readFileSync(
  ".github/workflows/staging-catalogue-preservation-rehearsal.yml",
  "utf8",
);
const sellerMigrationWorkflow = readFileSync(
  ".github/workflows/deploy-seller-inventory-migration.yml",
  "utf8",
);
const premiumSellerRuntimeWorkflow = readFileSync(
  ".github/workflows/manage-premium-seller-runtime.yml",
  "utf8",
);
...
const ingestionWorkflow = readFileSync(
  ".github/workflows/ingestion-workers.yml",
  "utf8",
);
for (const workflowName of readdirSync(".github/workflows").filter((name) =>
  ...
  assert.doesNotMatch(
    workflow,
    /uses:\s+actions\/(?:checkout|setup-node|setup-python)@v\d/,
    `${workflowName} must pin first-party actions`,
  );
}
...
assert.match(
  stagingWorkflow,
  /stackr-staging-physical-backup-diagnostics-\$\{\{ github\.run_id \}\}/,
);
assert.match(
  stagingWorkflow,
  /steps\.physical_backup_list\.outcome == 'failure'/,
);
assert.match(
  stagingWorkflow,
  /secret-scan\.mjs[\s\S]+stackr-backup-diagnostics/,
);
assert.match(
  stagingWorkflow,
...
assert.match(
  productionWorkflow,
  /stackr-production-physical-backup-diagnostics-\$\{\{ github\.run_id \}\}/,
);
assert.match(
  productionWorkflow,
  /steps\.physical_backup_list\.outcome == 'failure'/,
...
assert.deepEqual(
  [...stagingWorkflow.matchAll(/db push\s+--db-url "([^"]+)"/g)].map(
    (match) => match[1],
  ),
...
assert.deepEqual(
  [...productionWorkflow.matchAll(/db push\s+--db-url "([^"]+)"/g)].map(
    (match) => match[1],
  ),
  ["$STACKR_SOURCE_DB_URL", "$STACKR_SOURCE_DB_URL"],
...
assert.match(
  ...
  /STACKR_PRODUCTION_MONITOR_ENABLED == 'true'/,
);
...
assert.match(
  productionMonitorWorkflow,
  /if: failure\(\)[\s\S]+gh issue (?:comment|create)/,
);
...
assert.match(
  stagingWorkflow,
  /STACKR_DEPLOYMENT_SCOPE: \$\{\{ inputs\.release_scope \}\}/,
);
assert.match(
  stagingWorkflow,
...
assert.match(
  stagingWorkflow,
  /release_scope:[\s\S]+options: \[catalogue_api, recognition_service, full_platform\]/,
);
assert.match(stagingWorkflow, /STACKR_STORAGE_BACKUP_APPROVED/);
assert.match(
  stagingWorkflow,
  /verify-staging-migration-reconciliation\.mjs --require-aligned/,
);
assert.match(
  stagingWorkflow,
  /verify-staging-readiness-evidence\.mjs --require-catalogue-api-ready/,
);
assert.match(
  stagingWorkflow,
  /verify-staging-readiness-evidence\.mjs --require-release-ready/,
);
...
assert.match(
  stagingWorkflow,
  /Deploy recognition container[\s\S]+if: inputs\.release_scope != 'catalogue_api'/,
);
assert.match(
  stagingWorkflow,
  /Recognition-service-only deployments do not apply database migrations/,
);
assert.match(
  stagingWorkflow,
  /verify_only:[\s\S]+Verify the currently deployed recognition service without redeploying it/,
);
assert.match(
  stagingWorkflow,
  /if: inputs\.release_scope != 'catalogue_api' && !inputs\.verify_only/,
);
assert.match(
  stagingWorkflow,
  /Load the recognition smoke credential from Railway/,
);
assert.match(
  stagingWorkflow,
  /variable list --json[\s\S]+STACKR_RECOGNITION_GATEWAY_SERVICE_SECRET/,
);
assert.match(stagingWorkflow, /::add-mask::\$runtime_secret/);
assert.match(
  stagingWorkflow,
  /npm run deploy:smoke -- --gateway= --backend= --recognition="\$STACKR_RECOGNITION_URL" --signed-recognition/,
);
...
assert.match(
  stagingWorkflow,
  /secret-scan\.mjs --directory="\$diagnostics_dir"/,
);
assert.match(
  readFileSync("scripts/deploy/smoke.mjs", "utf8"),
  /recognition_signed_vector_lookup[\s\S]+exceptionType/,
);
assert.match(
  stagingWorkflow,
  /RECOGNITION_REQUIRED:\$\{\{ inputs\.release_scope/,
);
assert.match(stagingWorkflow, /--require-published-catalogue/);
assert.match(
  recoveryWorkflow,
  /inputs\.confirmation == 'RESTORE STAGING BACKUP'/,
);
...
assert.match(recoveryWorkflow, /secrets\.STACKR_GITHUB_STAGING_BACKUPS/);
assert.doesNotMatch(
  recoveryWorkflow,
  /vars\.SUPABASE_(?:PROJECT_REF|RESTORE_PROJECT_REF)/,
);
...
assert.match(recoveryWorkflow, /REHEARSE AND APPLY STAGING MIGRATIONS/);
assert.match(recoveryWorkflow, /inputs\.expected_commit_sha == github\.sha/);
assert.match(recoveryWorkflow, /verify-staging-migration-plan\.mjs/);
assert.match(
  recoveryWorkflow,
  /create-staging-migration-release-evidence\.mjs/,
);
assert.match(recoveryWorkflow, /--expected-count=7/);
assert.match(recoveryWorkflow, /backups list/);
assert.match(recoveryWorkflow, /db push[\s\S]+--dry-run/);
assert.match(recoveryWorkflow, /db lint/);
assert.doesNotMatch(recoveryWorkflow, /SUPABASE_PRODUCTION_DB_URL/);
...
assert.match(
  productionBaselineWorkflow,
  /inputs\.confirmation == 'CAPTURE PRODUCTION SCHEMA'/,
);
...
assert.match(
  productionBaselineWorkflow,
  /supabase_migrations schema absent on source/,
);
assert.match(productionBaselineWorkflow, /secret-scan\.mjs/);
assert.match(productionBaselineWorkflow, /retention-days: 1/);
assert.match(
  productionBaselineWorkflow,
  /rm -rf "\$RUNNER_TEMP\/stackr-production-baseline"/,
);
assert.doesNotMatch(
  productionBaselineWorkflow,
  /db push|migration repair|psql|SUPABASE_ACCESS_TOKEN/,
);
...
assert.match(
  ...
  /secrets\.SUPABASE_RESTORE_DB_URL/,
);
...
assert.match(
  baselineMigrationTrialWorkflow,
  /inputs\.confirmation == 'REPLAY MIGRATIONS ON RESTORE TARGET'/,
);
assert.match(
  baselineMigrationTrialWorkflow,
  /inputs\.confirmation == 'REHEARSE STAGING CATALOGUE TRANSFER'/,
);
assert.doesNotMatch(baselineMigrationTrialWorkflow, /pull_request:/);
assert.match(
  baselineMigrationTrialWorkflow,
  /prepare-isolated-reconciliation-url\.mjs/,
);
assert.match(
  baselineMigrationTrialWorkflow,
  /verify-production-schema-baseline\.mjs/,
);
...
assert.match(
  baselineMigrationTrialWorkflow,
  /inputs\.baseline_history_version/,
);
...
assert.match(
  baselineMigrationTrialWorkflow,
  /Verify restored production migration history/,
);
assert.match(
  baselineMigrationTrialWorkflow,
  /restored-baseline-migration-keys\.txt/,
);
assert.match(
  baselineMigrationTrialWorkflow,
...
assert.doesNotMatch(
  baselineMigrationTrialWorkflow,
  /isolated-production-storage-fixture\.sql/,
);
assert.ok(
  baselineMigrationTrialWorkflow.indexOf(
    "/trial/artifact/migration-history-data.sql",
  ) <
    baselineMigrationTrialWorkflow.indexOf(
      'db push --db-url "$STACKR_RESTORE_DB_URL" --include-all --dry-run',
    ),
);
assert.match(
  baselineMigrationTrialWorkflow,
...
assert.match(
  baselineMigrationTrialWorkflow,
  /db push --db-url "\$STACKR_RESTORE_DB_URL" --include-all/,
);
assert.match(
  baselineMigrationTrialWorkflow,
...
assert.doesNotMatch(
  baselineMigrationTrialWorkflow,
  /migration-count\.txt"\)" = '\d+'/,
);
assert.match(
  baselineMigrationTrialWorkflow,
  /rm -rf "\$RUNNER_TEMP\/stackr-baseline-trial"/,
);
assert.match(
  baselineMigrationTrialWorkflow,
  /rehearse-staging-catalogue-transfer\.mjs/,
);
assert.match(
  baselineMigrationTrialWorkflow,
  /rm -rf "\$RUNNER_TEMP\/stackr-catalogue-transfer"/,
);
assert.doesNotMatch(
  baselineMigrationTrialWorkflow,
  /SUPABASE_ACCESS_TOKEN|--linked/,
);
assert.match(
...
assert.match(
  catalogueTransferWorkflow,
  /deploy\/production-catalogue-promotion-tables\.json/,
);
assert.match(
  catalogueTransferWorkflow,
  /STACKR_TRANSFER_TABLE_CONFIG=\$table_config/,
);
...
assert.doesNotMatch(
  catalogueTransferWorkflow,
  /pull_request:|push:|SUPABASE_ACCESS_TOKEN|db push|migration repair/,
);
assert.match(
  sellerMigrationWorkflow,
  /inputs\.confirmation == 'APPLY PREMIUM SELLER ACCESS BOUNDARY'/,
);
assert.match(sellerMigrationWorkflow, /github\.ref == 'refs\/heads\/main'/);
assert.match(
  sellerMigrationWorkflow,
  /inputs\.expected_commit_sha == github\.sha/,
);
...
assert.match(
  sellerMigrationWorkflow,
  /prepare-postgres-urls\.mjs --source-only/,
);
assert.match(
  sellerMigrationWorkflow,
  /verify-seller-inventory-production-migration\.mjs --before/,
);
assert.match(
  sellerMigrationWorkflow,
  /verify-seller-inventory-production-migration\.mjs --after/,
);
...
const ansiMigrationPlan =
  "\u001b[1m20260813135412_premium_seller_access_boundary.sql\u001b[22m";
const cleanMigrationPlan = ansiMigrationPlan.replace(
  /\x1b\[[0-?]*[ -/]*[@-~]/g,
  "",
);
assert.deepEqual(
  [...cleanMigrationPlan.matchAll(/\b(\d{14}_[A-Za-z0-9_]+\.sql)\b/g)].map(
    (match) => match[1],
  ),
  ["20260813135412_premium_seller_access_boundary.sql"],
...
assert.deepEqual(
  [
    ...sellerMigrationWorkflow.matchAll(
      /db push\s*[\\\s\n]+--db-url "([^"]+)"/g,
    ),
  ].map((match) => match[1]),
  ["$STACKR_SOURCE_DB_URL", "$STACKR_SOURCE_DB_URL"],
...
assert.doesNotMatch(
  sellerMigrationWorkflow,
  /railway|wrangler|eas-cli|rehearse-staging-catalogue-transfer/,
);
...
assert.equal(
  [...premiumSellerRuntimeWorkflow.matchAll(/secrets\.SUPABASE_DB_URL/g)]
    .length,
  1,
...
assert.match(
  premiumSellerRuntimeWorkflow,
  /set-premium-seller-runtime\.mjs --validate-request/,
);
...
assert.doesNotMatch(
  premiumSellerRuntimeWorkflow,
  /SUPABASE_ACCESS_TOKEN|db push|railway|wrangler|eas-cli/,
);
assert.doesNotMatch(
  premiumSellerRuntimeWorkflow,
  /run:[^\n]*\$\{\{ inputs\./,
...
assert.match(
  premiumSellerQaIdentityWorkflow,
  /options:\s+\- preflight\s+\- provision\s+\- send_magic_link/,
);
assert.match(
  premiumSellerQaIdentityWorkflow,
  /github\.ref == 'refs\/heads\/main'/,
...
assert.match(
  premiumSellerQaIdentityWorkflow,
  /group: stackr-premium-seller-qa-identity/,
);
assert.doesNotMatch(
  premiumSellerQaIdentityWorkflow,
  /group: stackr-premium-seller-runtime/,
...
assert.match(
  premiumSellerQaIdentityWorkflow,
  /STACKR_WORKFLOW_SHA: \$\{\{ github\.sha \}\}/,
);
assert.match(
  premiumSellerQaIdentityWorkflow,
  /STACKR_RELEASE_SHA: \$\{\{ inputs\.release_commit_sha \}\}/,
);
assert.match(
  premiumSellerQaIdentityWorkflow,
  /STACKR_EXPECTED_COMMIT_SHA: \$\{\{ inputs\.expected_commit_sha \}\}/,
);
...
assert.match(
  premiumSellerQaIdentityWorkflow,
  /PREMIUM_SELLER_QA_EMAIL: \$\{\{ secrets\.PREMIUM_SELLER_QA_EMAIL \}\}/,
);
assert.match(
  premiumSellerQaIdentityWorkflow,
...
assert.match(
  premiumSellerQaIdentityWorkflow,
  /SUPABASE_PRODUCTION_SECRET_KEY: \$\{\{ secrets\.SUPABASE_PRODUCTION_SECRET_KEY \}\}/,
);
assert.match(
  premiumSellerQaIdentityWorkflow,
  /STACKR_SUPABASE_PUBLISHABLE_KEY: \$\{\{ secrets\.STACKR_SUPABASE_PUBLISHABLE_KEY \}\}/,
);
assert.match(
  premiumSellerQaIdentityWorkflow,
  /prepare-postgres-urls\.mjs --source-only/,
...
assert.equal(
  [...premiumSellerQaIdentityWorkflow.matchAll(/secrets\.SUPABASE_DB_URL/g)]
    .length,
  1,
...
assert.match(
  premiumSellerQaIdentityWorkflow,
  /manage-premium-seller-qa-identity\.mjs --validate-request/,
);
assert.match(
  premiumSellerQaIdentityWorkflow,
  /manage-premium-seller-qa-identity\.mjs/,
);
assert.match(premiumSellerQaIdentityWorkflow, /npm ci --ignore-scripts/);
assert.doesNotMatch(
  premiumSellerQaIdentityWorkflow,
  /db push|migration repair|railway|wrangler|eas-cli/,
);
assert.doesNotMatch(
  premiumSellerQaIdentityWorkflow,
  /run:[^\n]*\$\{\{ inputs\./,
...
const qaAppMetadata = expectedPremiumSellerQaAppMetadata(
  qaReleaseSha,
  qaProviderMetadata,
);
...
assert.equal(
  stagingAppConfig.status,
  0,
  stagingAppConfig.stderr || stagingAppConfig.stdout,
);
assert.equal(
  PREMIUM_SELLER_QA_REDIRECT_URL,
...
assert.throws(
  () => normalizeQaEmail(" release-qa@example.test"),
  /premium_seller_qa_email_invalid/,
);
assert.deepEqual(
  resolvePremiumSellerQaIdentityRequest({
    action: "preflight",
...
assert.doesNotThrow(() =>
  assertPublicPremiumSellerQaAuthSettings({ external: { email: true } }),
);
assert.throws(
    ...
    assertManagedPremiumSellerQaIdentity(
      {
        ...
        app_metadata: {
          ...qaAppMetadata,
          stackr_release_qa: { ...PREMIUM_SELLER_QA_MARKER, schema_version: 2 },
        },
      },
      { email: qaEmail, releaseSha: qaReleaseSha },
    ),
...
assert.throws(
  () =>
    assertManagedPremiumSellerQaIdentity(
      { ...managedQaUser, role: "service_role" },
      {
        email: qaEmail,
        releaseSha: qaReleaseSha,
      },
...
assert.throws(
  () =>
    assertManagedPremiumSellerQaIdentity(
      { ...managedQaUser, is_sso_user: true },
      {
        email: qaEmail,
        releaseSha: qaReleaseSha,
      },
    ...
    assertManagedPremiumSellerQaIdentity(
      {
        ...
        app_metadata: {
          ...qaAppMetadata,
          stackr_release_sha: qaPreviousReleaseSha,
        },
      },
      { email: qaEmail, releaseSha: qaReleaseSha },
    ),
...
assert.equal(
  selectSolePremiumSellerQaIdentity([managedQaUser], qaEmail)?.id,
  qaUserId,
);
...
assert.match(
  qaIdentityToolSource,
  /pg_advisory_xact_lock\(hashtext\('stackr\.premium_seller_runtime_control'\)\)/,
);
...
assert.doesNotMatch(
  qaIdentityToolSource,
  /allowPreviousRelease|updateUserById|deleteUser/,
);
assert.doesNotMatch(
  qaIdentityToolSource,
...
assert.match(
  ...
  /premium_seller_qa_confirmation_mismatch/,
);
assert.doesNotMatch(
...
assert.equal(
  safePremiumSellerQaIdentityFailureCode(
    new Error(`${qaSentinelEmail}:${qaSentinelSecret}`),
  ),
...
assert.match(
  reviewedPremiumSellerWrapper,
  /premium_seller_entitlement_required/,
);
assert.match(
  reviewedPremiumSellerWrapper,
  /auth\.jwt\(\) -> 'app_metadata' -> 'stackr_premium_seller'/,
);
assert.match(
  reviewedPremiumSellerWrapper,
  /private\.commit_seller_inventory_batch_impl/,
);
const reviewedAtomicSellerImplementation =
  loadReviewedAtomicSellerImplementationContract();
assert.match(
  reviewedAtomicSellerImplementation,
  /seller_inventory_authentication_required/,
);
assert.match(reviewedAtomicSellerImplementation, /pg_advisory_xact_lock/);
const runtimeToolSource = readFileSync(
...
assert.match(
  runtimeToolSource,
  /request\.action === 'enable_qa'[\s\S]+assertSellerLedgersEmpty/,
);
assert.match(
  runtimeToolSource,
  /request\.action === 'enable_qa'[\s\S]+assertPremiumSellerMigrationInstalled/,
);
...
const runtimeSecretUrl =
  "postgresql://postgres.oakdbbzdqwurpjnoqhmu:do-not-log@aws-0-eu-west-1.pooler.supabase.com:5432/postgres";
...
assert.ok(
  releaseManifest.components.database.privateSchemas.includes("private"),
);
assert.deepEqual(
  productionCataloguePromotion.excludedParentReferenceProjections,
  [
    {
...
const catalogueTransferScript = readFileSync(
...
assert.match(
  catalogueTransferScript,
  /delete from \$\{qualifiedName\(tableName\)\}/,
);
...
assert.match(
  catalogueTransferScript,
  /setUserTriggersEnabled\(target, tableName, true\)/,
);
...
assert.match(
  catalogueTransferScript,
  /if \(TRANSFER_MODE !== 'rehearse'\) \{[\s\S]+await target\.query\('commit'\);[\s\S]+targetCommitSucceeded = true/,
);
assert.match(
  catalogueTransferScript,
  /PROMOTE VERIFIED CATALOGUE TO PRODUCTION/,
);
assert.match(
  catalogueTransferScript,
  /production_promotion_target_guard_mismatch/,
);
assert.match(
  catalogueTransferScript,
  /invalid_transfer_source_identity_policy/,
);
assert.match(
  catalogueTransferScript,
  /production_source_identity_policy_mismatch/,
);
assert.match(
  catalogueTransferScript,
  /committed_source_identity_policy_mismatch/,
);
assert.match(
  catalogueTransferScript,
  /SOURCE_IDENTITY_POLICY === 'preserve_by_code'/,
...
assert.match(
  catalogueTransferScript,
  /jsonb_agg\(child_attribute\.attname::text/,
);
assert.doesNotMatch(
  catalogueTransferScript,
  /array_agg\(child_attribute\.attname/,
);
assert.match(
  catalogueTransferScript,
  /self_reference_transfer_requires_nullable_columns/,
);
assert.match(
  catalogueTransferScript,
  /source_identity_self_reference_unsupported/,
);
...
assert.match(
  catalogueTransferScript,
  /lockTransferTables\(target, tableConfig\.tables\)/,
);
...
assert.match(
  catalogueTransferScript,
  /if \(!targetAlreadyMatched\) \{[\s\S]+delete from \$\{qualifiedName\(tableName\)\}/,
);
assert.match(
  catalogueTransferScript,
...
assert.doesNotMatch(
  catalogueTransferScript,
  /throw new Error\(`target_commit_mismatch/,
);
assert.ok(
...
assert.doesNotMatch(
  catalogueTransferScript,
  /update catalog\.assets[\s\S]+set url/i,
);
assert.match(
  productionWorkflow,
...
const promotionAuditDirectory = mkdtempSync(
  path.join(tmpdir(), "stackr-promotion-audit-"),
);
try {
  const storageEvidencePath = path.join(
    promotionAuditDirectory,
    "storage-evidence.json",
  );
  ...
  for (const internalValue of [
    ...
    assert.doesNotMatch(
      serializedPublicAudit,
      new RegExp(internalValue.replace(".", "\\.")),
    );
  for (const internalValue of [
    "internal-staging-ref",
    "internal-production-ref",
  ...
  const postCommitMismatchEvidence = JSON.parse(
    readFileSync(databaseEvidencePath, "utf8"),
  );
  ...
  writeFileSync(
    postCommitMismatchPath,
    JSON.stringify(postCommitMismatchEvidence),
  );
  ...
  assert.match(
    postCommitMismatchAudit.stderr,
    /successful_database_no_op_not_verified/,
  );

  rmSync(databaseEvidencePath);
  const partialAuditOutputPath = path.join(
    promotionAuditDirectory,
    "partial-audit.json",
  );
  ...
  assert.match(
    invalidSuccessAudit.stderr,
    /successful_catalogue_promotion_evidence_missing/,
  );
  ...
  assert.match(
    contradictorySuccessAudit.stderr,
    /successful_database_no_op_not_verified/,
  );

  const badStorageEvidence = JSON.parse(
    readFileSync(storageEvidencePath, "utf8"),
  );
  badStorageEvidence.ok = false;
  badStorageEvidence.providerRequestsPerformed = true;
  const badStorageEvidencePath = path.join(
    promotionAuditDirectory,
    "bad-storage-evidence.json",
  );
  ...
  assert.match(
    badStorageSuccessAudit.stderr,
    /successful_storage_no_op_not_verified/,
  );
} finally {
...
const { prepareCatalogueSelfReferenceTransfer } =
...
const projectedExcludedParentReferences =
  projectCatalogueExcludedParentReferences(
...
assert.deepEqual(projectedExcludedParentReferences.projectedColumns, [
  "raw_record_id",
]);
...
for (const [foreignKeyPatch, expectedError] of [
  [
    { allColumnsNullable: false },
    /excluded_parent_reference_requires_nullable_columns/,
  ],
  [
    { deleteAction: "RESTRICT" },
    /excluded_parent_reference_requires_on_delete_set_null/,
  ],
]) {
  assert.throws(
    () =>
  ...
  [
    {
      ...
      code: "shared",
      display_name: "Production shared",
    },
    { id: "production-only", code: "legacy", display_name: "Production only" },
  ],
);
assert.equal(
  sourceIdentityPlan.sourceIdMap.get("staging-shared"),
  "production-shared",
);
...
assert.deepEqual(
  sourceIdentityPlan.preservedTargetOnlyRows.map(({ id, code }) => ({
    id,
    code,
  })),
...
assert.ok(
  sourceIdentityPlan.mappedSourceRows.some(
    ({ id }) => id === retainedProductionConflict.source_id,
  ),
...
const rerunSourceIdentityPlan = planCatalogueSourceIdentityMerge(
  ...
  [
    ...sourceIdentityPlan.mappedSourceRows,
    ...sourceIdentityPlan.preservedTargetOnlyRows,
  ],
);
assert.deepEqual(
...
assert.deepEqual(
  remappedSourceForeignKeys.rows.map(({ id, source_id: sourceId }) => ({
    id,
    sourceId,
  })),
  [
...
const reorderedCompositeKeySourceRows = [
  {
    catalogue_version_id: "version-1",
    source_id: "z-source",
    external_id: "one",
  },
  {
...
assert.equal(
  assetRewrite.rows[1].url,
  "https://staging-ref.example/remote.webp",
);
assert.equal(
...
assert.equal(
  idempotentAssetRewrite.rows[0].updated_at,
  priorProductionAssetTimestamp,
);
...
assert.equal(
  changedAssetMetadataRewrite.rows[0].updated_at,
  assetRewriteTimestamp,
);
...
assert.deepEqual(catalogueTransferTargetMatch(exactTargetMatchInput), {
  matches: true,
  reason: null,
});
assert.deepEqual(
...
assert.deepEqual(
  catalogueTransferTargetMatch({
    ...
    primaryKey: ["id"],
    targetSequenceStates: [
      { ...sequenceStates[0], lastValue: "2", isCalled: true },
    ],
  }),
  { matches: false, reason: "sequence_state" },
);

const { normalizePostgresUrl } =
  await import("./deploy/prepare-postgres-urls.mjs");
const {
...
const { assertNoPostgresConnectionOverrides } =
  await import("./deploy/postgres-url-guard.mjs");
...
const tlsConfiguredUrl =
  "postgresql://postgres.exampleproject:password@aws-0-eu-west-1.pooler.supabase.com:5432/postgres?sslmode=require&sslrootcert=%2Ftmp%2Funtrusted-ca.pem&sslcert=%2Ftmp%2Fclient.pem&sslkey=%2Ftmp%2Fclient.key&ssl=true&application_name=preserved";
...
for (const parameter of [
  ...
  assert.equal(
    verifiedPostgresUrl.searchParams.has(parameter),
    false,
    `TLS URL parameter ${parameter} must not override the pinned certificate`,
  );
}
assert.equal(
  verifiedPostgresUrl.searchParams.get("application_name"),
  "preserved",
);
...
assert.throws(
  () =>
    assertNoPostgresConnectionOverrides(
      new URL(
        "postgresql://postgres:password@aws-0-eu-west-1.pooler.supabase.com:5432/postgres?user=postgres.anotherproject",
      ),
    ),
...
assert.equal(
  verifiedPostgresClient.connectionParameters.ssl.rejectUnauthorized,
  true,
);
assert.equal(
  new X509Certificate(verifiedPostgresClient.connectionParameters.ssl.ca)
    .fingerprint256,
  SUPABASE_ROOT_CA_FINGERPRINT256,
...
for (const clientScript of [
  ...
  assert.doesNotMatch(
    source,
    /rejectUnauthorized\s*:\s*false|sslmode=no-verify|NODE_TLS_REJECT_UNAUTHORIZED|uselibpqcompat/,
  );
}
...
assert.equal(
  isStorageConnectionLimitError(
    new Error("Too many connections issued to the database"),
  ),
  true,
);
assert.equal(
  isStorageConnectionLimitError(new Error("temporary network error")),
  false,
);
assert.equal(
  isStorageThrottleError({ statusCode: 429, message: "SlowDown" }),
  true,
);
assert.equal(
  isStorageThrottleError({ cause: { code: "too_many_connections" } }),
  true,
);
assert.equal(
  isRetryableStorageError({ status: 503, message: "service unavailable" }),
  true,
);
assert.equal(
  isRetryableStorageError({ code: "ECONNRESET", message: "socket closed" }),
  true,
);
assert.equal(
  isRetryableStorageError({ statusCode: 401, message: "invalid key" }),
  false,
);
assert.equal(
  isRetryableStorageError({ statusCode: 404, message: "object missing" }),
  false,
);
const abortedUploadError = {
  statusCode: 400,
  message:
    "upload_production_object:public/card_image/hash/original.png:Bad Request",
};
assert.equal(isRetryableStorageError(abortedUploadError), false);
assert.equal(
  isRetryableStorageError(abortedUploadError, {
...
await assert.rejects(
  retryStorageOperation(
    ...
    },
    {
      attempts: 6,
      wait: async () =>
        assert.fail("fatal Storage errors must not be delayed or retried"),
    },
  ),
  /invalid service key/,
);
  ...
  retryStorageOperation(
    async () => {
      ...
      const error = new Error(
        "SlowDown: Too many connections issued to the database",
      );
...
const sourceOnlyValidation = run(
  "scripts/deploy/prepare-postgres-urls.mjs",
  ["--source-only"],
  {
...
assert.equal(
  sourceOnlyValidation.status,
  0,
  sourceOnlyValidation.stderr || sourceOnlyValidation.stdout,
);
assert.match(
  sourceOnlyValidation.stdout,
  /Protected source database URL verified\./,
);
assert.doesNotMatch(sourceOnlyValidation.stdout, /p=a@#ss%word/);

const sourceOnlyEnvironmentTemp = mkdtempSync(
  path.join(tmpdir(), "stackr-source-url-test-"),
);
try {
  const sourceOnlyEnvironmentPath = path.join(
    sourceOnlyEnvironmentTemp,
    "github.env",
  );
  const sourceOnlyExport = run(
    "scripts/deploy/prepare-postgres-urls.mjs",
    ["--source-only"],
    {
      SUPABASE_DB_URL: rawPasswordUrl.normalized,
  ...
  assert.equal(
    sourceOnlyExport.status,
    0,
    sourceOnlyExport.stderr || sourceOnlyExport.stdout,
  );
  assert.equal(
...
const baselineUrlTemp = mkdtempSync(
  path.join(tmpdir(), "stackr-baseline-url-test-"),
);
try {
  const baselineEnvironmentPath = path.join(baselineUrlTemp, "github.env");
  const { prepareProductionBaselineUrl } =
    await import("./deploy/prepare-production-baseline-url.mjs");
  const preparedBaseline = prepareProductionBaselineUrl({
    connectionString:
      "postgresql://postgres.productionref:p=a@#ss@aws-0-eu-west-1.pooler.supabase.com:5432/postgres",
...
const reconciliationUrlTemp = mkdtempSync(
  path.join(tmpdir(), "stackr-reconciliation-url-test-"),
);
try {
  const reconciliationEnvironmentPath = path.join(
    reconciliationUrlTemp,
    "github.env",
  );
  const { prepareIsolatedReconciliationUrl } =
    await import("./deploy/prepare-isolated-reconciliation-url.mjs");
  const preparedReconciliation = prepareIsolatedReconciliationUrl({
    connectionString:
      "postgresql://postgres.restoreref:p=a@#ss@aws-0-eu-west-1.pooler.supabase.com:5432/postgres",
  ...
  assert.throws(
    () =>
      prepareIsolatedReconciliationUrl({
        connectionString:
          "postgresql://postgres.productionref:password@aws-0-eu-west-1.pooler.supabase.com:5432/postgres",
        projectRef: "productionref",
        productionProjectRef: "productionref",
        stagingProjectRef: "stagingref",
        environmentPath: reconciliationEnvironmentPath,
...
const { createSchemaBaselineEvidence } =
  await import("./deploy/create-schema-baseline-evidence.mjs");
...
const absentHistoryEvidence = createSchemaBaselineEvidence({
  ...
  historyData:
    "-- stackr: no migration history rows because schema is absent\n",
});
assert.equal(
  absentHistoryEvidence.inventory.migrationHistorySchemaPresent,
  false,
);
...
const { sanitizeRoleDumpText } =
  await import("./deploy/sanitize-supabase-role-dump.mjs");
const roleDump = [
...
assert.doesNotMatch(
  sanitizedRoleDump.output,
  /^GRANT "stackr_ingest" TO "supabase_admin"/m,
);
assert.equal(
...
const { buildRestoreCleanupSql, buildRestoreCleanupSqlWithRoles } =
...
assert.doesNotMatch(
  cleanupWithRoles.sql,
  /DROP ROLE IF EXISTS "stackr_recognition"/,
);
assert.ok(
...
assert.match(
  productionWorkflow,
  /release_scope:[\s\S]+options: \[catalogue_api, full_platform\]/,
);
assert.match(productionWorkflow, /--require-catalogue-api-ready/);
assert.match(
  productionWorkflow,
  /Catalogue API promotion currently supports the guarded first-release bootstrap only/,
);
assert.match(
  productionWorkflow,
  /Catalogue API bootstrap does not publish a mobile update/,
);
assert.match(
  productionWorkflow,
  /Remove a failed first production gateway[\s\S]+wrangler --cwd gateway delete --env production --force/,
);
...
assert.match(
  ingestionWorkflow,
  /STACKR_CATALOGUE_INGESTION_AUTOMATION_APPROVED/,
);
assert.match(ingestionWorkflow, /--setId="\$STACKR_INGEST_SET"/);
assert.match(
  ingestionWorkflow,
  /resume-import[\s\S]+--runKey="\$STACKR_INGEST_ID"/,
);
assert.match(
  ingestionWorkflow,
  /rebuild-record[\s\S]+--providerRecordId="\$STACKR_INGEST_ID"/,
);
assert.doesNotMatch(
  rollbackWorkflow,
...
assert.notEqual(
  ...
  0,
  "backup verification must fail closed without backup files",
);
assert.doesNotMatch(
  backupFailure.stderr,
  /ENOENT|TypeError/,
  "backup verifier should report missing evidence cleanly",
);
...
try {
  ...
  writeFileSync(
    payloadPath,
    JSON.stringify([{ group: groupId }, { group: groupId }]),
  );
