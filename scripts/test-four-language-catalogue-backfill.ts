...
import {
  ...
  FOUR_LANGUAGE_BATCH_PROGRESS_EVENT,
...
assert.equal(completedProgressLine.includes('\n'), false, 'batch progress must remain one-line JSON');
assert.deepEqual(JSON.parse(completedProgressLine), {
  ...
  failed: 0,
  batchesSuccessful: 2,
  successfulBatchPercent: 50,
  batchesTerminal: 2,
  terminalBatchPercent: 50,
  durationMs: 1234,
  attempts: 2,
  status: 'completed',
});
assert.deepEqual(JSON.parse(formatBatchProgressLog({
  language: 'zh-cn',
  offset: 2000,
  batchesPlanned: 4,
  completed: 1,
  ...
  failed: 0,
  batchesSuccessful: 3,
  successfulBatchPercent: 75,
  batchesTerminal: 3,
  terminalBatchPercent: 75,
  durationMs: 9,
  attempts: 0,
  status: 'already_completed',
});
