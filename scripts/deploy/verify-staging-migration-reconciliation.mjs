...
const explicitEvidencePath = process.argv
  .find((arg) => arg.startsWith("--evidence="))
  ?.slice(11);
const latestEvidenceFile = readdirSync("deploy/evidence")
  .filter((name) =>
    /^staging-migration-reconciliation-\d{4}-\d{2}-\d{2}\.json$/.test(name),
  )
  .sort()
  .at(-1);
const evidencePath =
...
if (!evidencePath)
  throw new Error(
    "No staging migration reconciliation evidence file was found.",
  );
...
function orderedMigrationKeySha256(migrations) {
  return createHash("sha256")
    .update(
      `${migrations.map((name) => name.replace(/\.sql$/, "")).join("\n")}\n`,
    )
    .digest("hex");
}
...
if (
  evidence.stagingProjectRef !== manifest.components.database.stagingProjectRef
) {
  errors.push("staging_project_ref_mismatch");
}
if (evidence.productionMutationPerformed !== false)
  errors.push("production_mutation_not_prohibited");
...
if (
  ...
  if (
    evidence.orderedMigrationKeySha256 !== keyDigest ||
  ...
  if (
    ...
    const baselineHistoryCount =
      evidence.schemaVersion === "stackr-migration-reconciliation-v1.3.0"
    ...
    if (
      !Number.isInteger(baselineHistoryCount) ||
    ...
    if (
      evidence.baseline?.migrationHistoryRestored !== true ||
      evidence.baseline?.restoredMigrationHistoryCount !==
        baselineHistoryCount ||
      evidence.baseline?.exactRepositoryPrefixMatch !== true ||
      evidence.baseline?.pendingRepositoryMigrationCount !==
        pendingMigrationCount
    ) {
      errors.push("baseline_migration_history_restore_not_verified");
    }
    if (
      evidence.isolatedCandidate?.baselineMigrationHistoryRestored !== true ||
      evidence.isolatedCandidate?.pendingRepositoryMigrationsApplied !== true ||
      evidence.isolatedCandidate?.pendingRepositoryMigrationCount !==
        pendingMigrationCount ||
    ...
    if (
      ...
      (evidence.baseline?.source !== "staging" ||
        ...
        evidence.isolatedCandidate?.rollbackVerified !== true ||
        evidence.isolatedCandidate?.securityLintPassed !== true ||
        evidence.staging?.migrationHistoryAligned !== true ||
        evidence.staging?.securityLintPassed !== true ||
        evidence.stagingMutationPerformed !== true)
    ) {
      errors.push("staging_migration_release_evidence_incomplete");
    }
  }
  if (requireAligned && evidence.status !== "aligned")
    errors.push("migration_history_not_aligned");
    ...
    JSON.stringify(
      {
        ...
        stagingMigrationHistoryCount:
          evidence.stagingMigrationHistoryCountAfter,
...
if (
  ...
  evidence.matchedRepositoryMigrations.length +
    evidence.stagingOnlyMigrationNames.length
) {
  errors.push("staging_migration_count_inconsistent");
}
if (
  evidence.unverifiedRepositoryMigrationCount !==
...
if (
  evidence.repositoryMigrationGroups?.preCanonicalLegacyCount +
    evidence.repositoryMigrationGroups?.canonicalAndLaterCount !==
  localMigrations.length
) {
  errors.push("repository_migration_group_count_inconsistent");
}
if (
  evidence.repositoryMigrationGroups?.accountedForCount !==
  evidence.matchedRepositoryMigrations.length
) {
  errors.push("accounted_repository_migration_count_inconsistent");
}
if (
  evidence.dryRun?.wouldPushCount !== localMigrations.length ||
  evidence.dryRun?.safeToApply !== false
) {
  errors.push("migration_dry_run_evidence_inconsistent");
}
if (evidence.status === "blocked_missing_pre_repository_baseline") {
  if (
    evidence.reconciliationComplete !== false ||
    evidence.baselineGap?.confirmed !== true
  ) {
...
if (
  evidence.status === "isolated_candidate_aligned_staging_promotion_blocked"
) {
  if (
    evidence.reconciliationComplete !== false ||
  ...
  if (
    ...
    evidence.isolatedCandidate?.repositoryMigrationCount !==
      localMigrations.length ||
  ...
  if (
    evidence.stagingMutationPerformed !== false ||
    evidence.isolatedBranchMutationPerformed !== true
  ) {
    errors.push("isolated_candidate_mutation_scope_invalid");
  }
  if (
    (evidence.isolatedCandidate?.securityAdvisorErrorCount ?? -1) !== 0 ||
    evidence.isolatedCandidate?.securityAdvisorLastSuccessfulMigrationCount !==
      localMigrations.length ||
  ...
  if (
    preservation?.status !== "rehearsed_and_rolled_back" ||
...
for (const match of evidence.matchedRepositoryMigrations) {
  ...
  if (
    lfSha256(`supabase/migrations/${match.repositoryFile}`) !==
    match.repositoryLfSha256
  ) {
    errors.push(
      `matched_repository_migration_hash_drift:${match.repositoryFile}`,
    );
  }
}
  ...
  JSON.stringify(
    {
      ...
      unverifiedRepositoryMigrationCount:
        evidence.unverifiedRepositoryMigrationCount,
      errors,
    },
    null,
