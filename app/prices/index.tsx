...
export default function MarketScreen() {
  ...
  }, [theme.colors.text, theme.colors.textSoft, watchlistPriceMap]);
  ...
  const renderLookupOption = useCallback((option: { key: LookupType; label: string; icon: keyof typeof Ionicons.glyphMap }) => {
    ...
    const iconAsset = listingCategoryIcons[option.key];
    return (
      <TouchableOpacity
        key={option.key}
        onPress={() => {
  ...
  const selectedLookupIconAsset = listingCategoryIcons[selectedLookupOption.key];
          ...
          <View style={{ paddingHorizontal: 16, paddingTop: 8, paddingBottom: 8 }}>
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flex: 1 }}>
                ...
                >
                  {selectedLookupIconAsset ? (
                    <StackrCardActionIcon
                      source={selectedLookupIconAsset}
                      frameSize={stackrSellCategoryIconSizes.selectedPickerFrame}
                      artworkSize={stackrSellCategoryIconSizes.selectedPickerArtwork}
                    />
                  ) : (
                  ...
                  <>
                    ...
                    <View style={{ backgroundColor: theme.colors.card, borderRadius: 22, padding: 16, borderWidth: 1, borderColor: theme.colors.border, ...cardShadow }}>
                        ...
                        {(['raw_card', 'graded_slab'] as LookupType[]).map((mode) => {
                          ...
                          return (
                            <TouchableOpacity
                              ...
                              style={{ flex: 1, borderRadius: 12, paddingVertical: 10, alignItems: 'center', backgroundColor: active ? theme.colors.primary + '12' : theme.colors.surface, borderWidth: 1, borderColor: active ? theme.colors.primary : theme.colors.border }}
                            >
                            ...
                            {GRADING_COMPANIES.map((company) => {
                              ...
                              return (
                                <TouchableOpacity
                                  ...
                                  style={{ borderRadius: 999, paddingHorizontal: 12, paddingVertical: 8, backgroundColor: active ? `${accent}18` : theme.colors.surface, borderWidth: 1, borderColor: active ? accent : theme.colors.border }}
                                >
                                  <Text style={{ color: active ? accent : theme.colors.text, fontWeight: '800', fontSize: 12 }}>{company}</Text>
                                </TouchableOpacity>
                              );
                            })}
                          </View>
                          ...
                          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
                            {GRADES.map((value) => (
                              <TouchableOpacity
                                ...
                                style={{ borderRadius: 999, paddingHorizontal: 12, paddingVertical: 8, backgroundColor: grade.trim() === value ? theme.colors.primary + '12' : theme.colors.surface, borderWidth: 1, borderColor: grade.trim() === value ? theme.colors.primary : theme.colors.border }}
                              >
                                <Text style={{ color: grade.trim() === value ? theme.colors.primary : theme.colors.text, fontWeight: '800', fontSize: 12 }}>{value}</Text>
                              </TouchableOpacity>
                            ))}
                          </View>
                          <TextInput
                          ...
                          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                            {RAW_CONDITIONS.map((condition) => (
                              <TouchableOpacity
                                ...
                                style={{ borderRadius: 999, paddingHorizontal: 12, paddingVertical: 8, backgroundColor: rawCondition === condition ? theme.colors.primary + '12' : theme.colors.surface, borderWidth: 1, borderColor: rawCondition === condition ? theme.colors.primary : theme.colors.border }}
                              >
                                <Text style={{ color: rawCondition === condition ? theme.colors.primary : theme.colors.text, fontWeight: '800', fontSize: 12 }}>{condition}</Text>
                              </TouchableOpacity>
                            ))}
                          </View>
                        </>
                      ...
                      <PokeTraceMarketInsights
                        ...
                        grade={lookupType === 'graded_slab' ? grade.trim() || '10' : null}
                      />
