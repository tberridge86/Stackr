...
import { sha256Text } from './collectionRequestHash';
...
function collectionBatchIdentityKey(input: {
  cardId: string;
  setId: string;
  language?: PokemonCardLanguage | string | null;
}) {
  return [
    normalizePokemonCardLanguage(input.language),
    String(input.setId).trim().toLowerCase(),
    String(input.cardId).trim().toLowerCase(),
  ].join(':');
}
...
export function aggregateCollectionBatch(
  ...
  for (const card of cards) {
    ...
    const key = collectionBatchIdentityKey({ cardId, setId, language });
    ...
    if (existing) {
      entries.set(key, {
        ...
        imageUrl: existing.imageUrl ?? cleanText(stripTcgdexReferenceBeforePersistence(card.imageUrl)),
    ...
    entries.set(key, {
      ...
      imageUrl: cleanText(stripTcgdexReferenceBeforePersistence(card.imageUrl)),
  ...
  return [...entries.values()].sort((left, right) => (
    collectionBatchIdentityKey(left).localeCompare(collectionBatchIdentityKey(right))
  ));
}
...
function indexBinderRows(rows: BinderCardRow[]) {
  const byIdentity = new Map<string, BinderCardRow>();
  for (const row of rows) {
    const key = collectionBatchIdentityKey({
      cardId: row.card_id,
      setId: row.set_id,
      language: row.language,
    });
    if (byIdentity.has(key)) {
      throw new Error('Binder contains duplicate exact card identities and must be repaired before adding a batch.');
    }
...
function expectedBinderStateMatches(
  rows: BinderCardRow[],
  expected: readonly ExpectedBinderCard[],
  batch: readonly CollectionBatchEntry[],
) {
  let byIdentity: Map<string, BinderCardRow>;
  try {
    byIdentity = indexBinderRows(rows);
  } catch {
    return false;
  }
  return expected.every((item, index) => {
    ...
    const row = byIdentity.get(collectionBatchIdentityKey(entry));
    if (!row) return false;
...
function baselineBinderStateMatches(
  rows: BinderCardRow[],
  baseline: readonly BaselineBinderCard[],
  batch: readonly CollectionBatchEntry[],
) {
  ...
  return baseline.every((item, index) => {
    const entry = batch[index];
    if (!entry) return false;
    const row = byIdentity.get(collectionBatchIdentityKey(entry));
...
function buildCollectionBatchPlan(input: {
  ...
  const existingByIdentity = indexBinderRows(input.existingRows);
  ...
  for (const entry of input.batch) {
    const existing = existingByIdentity.get(collectionBatchIdentityKey(entry)) ?? null;
    ...
    const row = Object.freeze({
      ...
      // A newly resolved reference stays display-only. Do not let it replace
      // (or erase) an image pointer that was already stored on this row.
      image_url: preserveExistingImageUrlBeforePersistence(
        entry.imageUrl,
        existing?.image_url,
      ),
...
async function saveCollectionBatchOnce(input: {
  ...
  if (storedJournal) {
    const decision = decideCollectionBatchJournalRecovery({
      ...
      baselineMatches: baselineBinderStateMatches(existingRows, storedJournal.baseline, input.batch),
      expectedMatches: expectedBinderStateMatches(existingRows, storedJournal.expected, input.batch),
    });
    if (decision === 'block') {
      throw new CollectionBatchReconciliationRequiredError(input.requestKey);
    }
