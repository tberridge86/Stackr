...
function canonicalRequestCards(cards: readonly CollectionBatchRequestCard[]) {
  ...
  for (const card of cards) {
    ...
    const language = cleanText(card.language)?.toLowerCase() ?? 'en';
    const key = [language, setId.toLowerCase(), cardId.toLowerCase()].join(':');
  ...
  return [...grouped.values()]
    .sort((left, right) => (
      [left.language, left.setId, left.cardId].join(':')
        .localeCompare([right.language, right.setId, right.cardId].join(':'))
    ))
